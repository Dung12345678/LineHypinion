...
namespace BMS
{
	public partial class frmShapeDetails : _Forms
	{
		...
		bool ValidateForm()
		{
			...
			{
				dt = TextUtilsHP.Select("select 1 from KnifeDetailList where KnifeCode = '" + txtShapeCode.Text.Trim() + "' and ID <> " + _KnifeDetailListModel.ID);
			}
			else
			{
				dt = TextUtilsHP.Select("select 1 from KnifeDetailList where KnifeCode = '" + txtShapeCode.Text.Trim() + "'");
			}
			if (dt != null)
			{
				if (dt.Rows.Count > 0)
