...
namespace BMS
{
	public partial class frmImportMotorNew : _Forms
	{
		...
		void LoadSequence()
		{
			...
			using (WaitDialogForm fWait = new WaitDialogForm("Vui lòng chờ trong giây lát", ""))
			{
				...
				DataRow[] dr = dt2.Select($"ShelfCode='{txtLocation.Text.Trim()}'");
				if (dr.Length > 0)
				{
					string ColumnName = TextUtilsStock.ToString(dr[0]["SequenceCode"]);
					DataRow []drrrr  = dt2.Select($"SequenceCode='{ColumnName}'");
					if(drrrr.Length>0)
					{
						dtShelf = drrrr.CopyToDataTable();
					}	
				}
				...
				for (int i = 0; i < dtShelf.Rows.Count; i++)
				{
					...
					if (i > 0)
					{
						SequenceIDOld = TextUtilsStock.ToInt(dtShelf.Rows[i - 1]["SequenceID"]);
					}
					SequenceIDNew = TextUtilsStock.ToInt(dtShelf.Rows[i]["SequenceID"]);
					...
					if (SequenceIDNew != SequenceIDOld)
					{
						...
						tblLayout.Controls.Add(uc, columnSequence + 1, 0);
					...
					uc1.NameShelf = TextUtilsStock.ToString(dtShelf.Rows[i]["ShelfName"]);
					uc1.Qty = TextUtilsStock.ToInt(dtShelf.Rows[i]["ShelfNumber"]);
					uc1.Dock = DockStyle.Fill;
					tblLayout.Controls.Add(uc1, column, row);
					_lstUCShelf.Add(uc1);
					_lstShelfName.Add(TextUtilsStock.ToString(dtShelf.Rows[i]["ShelfName"]));
				}
