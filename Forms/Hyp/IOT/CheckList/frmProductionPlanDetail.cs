	...
	public partial class frmProductionPlanDetail : _Forms
	{
		...
		private bool ValidateForm()
		{
			...
			else
			{
				...
				{
					dt = LibIE.Select("select top 1 ProductionPlan from Product where ProductCode = '" + txtProductcode.Text.Trim() + "' and ID <> " + ProductionPlan.ID);
				}
				else
				{
					dt = LibIE.Select("select top 1 ProductionPlan from Product where ProductCode = '" + txtProductcode.Text.Trim() + "'");
				}
				if (dt != null)
				{
					if (dt.Rows.Count > 0)
